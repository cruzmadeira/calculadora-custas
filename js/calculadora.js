 ...
 * calculadora.js — núcleo de cálculo da Tabela de Custas (v3, set/2026)
...
const TABELA = {
  ...
  admin: {
    base: 30000.0,              // parcela fixa até R$ 1 milhão
    faixas: [                   // [limiteSuperior, alíquota marginal]
      [5_000_000, 0.016],
      [20_000_000, 0.007],
      [50_000_000, 0.0035],
      [100_000_000, 0.0018],
...
function marginal(valor, base, piso1M, faixas, teto) {
  if (!(valor > 0)) return 0;
  let total = base;
  let anterior = piso1M;
...
function taxaAdministracao(valor) {
  ...
  return marginal(valor, a.base, a.piso1M, a.faixas, a.teto);
}

/** Honorários de árbitro único — procedimento comum. */
function honorariosUnico(valor) {
  const h = TABELA.honorarios;
  return marginal(valor, h.base, h.piso1M, h.faixas, h.teto);
}
